...
import gg.jominsubyungsin.domain.repository.TagListRepository;
...
public class TagServiceImpl implements TagService{
	...
	public List<SelectProjectDto> projectList(List<String> tags, UserEntity user, Pageable pageable) {
		...
		try{
			...
			System.out.println(project_ids.toString());
			List<ProjectEntity> projectEntities = projectRepository.findAllByIdIsInOOrderByIdDesc(project_ids);
			for(ProjectEntity projectEntity: projectEntities){
				...
				projects.add(new SelectProjectDto(projectEntity, users));
			}
			return projects;
		}catch (Exception e){
			e.printStackTrace();
