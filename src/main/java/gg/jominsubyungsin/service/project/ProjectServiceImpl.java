...
public class ProjectServiceImpl implements ProjectService {
	...
	public void saveProject(GetProjectDto projectDto, UserEntity mainUser) throws IOException {
		if (projectDto.getFiles() == null || projectDto.getFiles().isEmpty()) {
			throw new HttpClientErrorException(HttpStatus.BAD_REQUEST, "파일은 무조건 1개 이상 보내야 합니다");
		}
		...
		try {
			...
			for (Long id : projectDto.getUsers()) {
				...
				if (userEntities.contains(saveUser)) {
					throw new HttpClientErrorException(HttpStatus.BAD_REQUEST, "같은 유저 입니다");
				}
	...
	@Override
	@Transactional(readOnly = true)
	public List<SelectProjectDto> getProjects(Pageable pageable, UserEntity me, String value) {
		List<SelectProjectDto> projectDtos = new ArrayList<>();
		value = "%" + value + "%";
		try {
			Page<ProjectEntity> projectEntityPage = projectListRepository.findByOnDeleteAndTitleLikeOrContentLike(false, value, value, pageable);
	...
	public Long countProject(UserEntity user) {
		try {
			return projectUserConnectRepository.countByUserAndGetOut(user, false);
		} catch (Exception e) {
			throw e;
		}
	}
	...
	@Override
	@Transactional(readOnly = true)
	public Long countProject(String title) {
		title = "%" + title + "%";
		try {
			return projectRepository.countByTitleLikeAndOnDelete(title, false);
		} catch (Exception e) {
			throw e;
		}
	}
